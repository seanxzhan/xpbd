...
def test_gauss_seidel_converges_faster():
    """Gauss-Seidel achieves lower residual than Jacobi at same iter count."""
    N = 20
    spacing = 0.5
    X = np.zeros((N, 3))
    X[:, 0] = np.arange(N) * spacing
    ...
    for solver_name in ("jacobi", "gauss-seidel"):
        masses = np.ones(N)
        ...
        rest = np.full(N - 1, spacing)
        sys.add_constraint(Stretch(edges, rest, compliance=0.0))
        sys.step(dt=1e-3, iters=iters, solver=solver_name)
        lengths = np.linalg.norm(np.diff(sys.X, axis=0), axis=1)
        if solver_name == "jacobi":
            res_jacobi = float(np.max(np.abs(lengths - spacing)))
        else:
            res_gs = float(np.max(np.abs(lengths - spacing)))

    assert res_gs <= res_jacobi + 1e-12, (
        f"GS ({res_gs}) should be <= Jacobi ({res_jacobi})"
    )
